...
Our objective is simple. \textit{Can a Question Answering (QA) system keep a secret?} Specifically: 

\begin{enumerate}
    \item How do we represent secrets and detect if a secret might leak during QA?
    \item Is there any relationship between secret keeping, QA accuracy and runtime?
    \item What causes false positive responses (mistakenly thinking something is a secret)?
    \item What causes false negative responses (mistakenly leaking secret information)?. 
\end{enumerate}
...
ChatGPT's arrival shows \cite{samczsun2022} how a human can employ an adversarial questioning method for truth amplification\cite{Evans2021} we call \textit{interrogation} to induce information leakage from a system\cite{FAIR†2022}. Current content moderation approaches are ineffective in resisting \textit{interrogation}, because the model doesn't actually know what information it shouldn't disclose. We need to teach the system to keep certain information 'secret' while under interrogation, or better yet, use covert omission and tactical misdirection to prevent interrogation from occurring. Our experiments compare four models of 'secret-keeping' across two architectures and identify focal points for future work.
}
...
\includegraphics[width=0.6\linewidth]{figures/secret_keeper_diagram.PNG}
\captionof{figure}{Approach 2 uses word embeddings associated with the secret to purge secret material from the source of the QA system. It can't leak information it doesn't have.}
