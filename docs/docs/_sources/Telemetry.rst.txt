...
All files relating to the measurement of performance of each Graph
Problem against each Dataset
...
**Performance** Measured in Giga Traversed Edges Per Second `Source:
HIVE Workload Analysis Report pg
15 <https://drive.google.com/file/d/1qM5POYo0vB9p-QKb88oNzzED2a7cJdW_/view?usp=drive_link>`__

**Efficency** Measured in Giga Traversed Edges Per Second Per Watt
`Source: HIVE Workload Analysis Report pg
15 <https://drive.google.com/file/d/1qM5POYo0vB9p-QKb88oNzzED2a7cJdW_/view?usp=drive_link>`__

Survey of Metrics
-----------------

Add in links / descriptions of metrics we can / should use. Sourcing
...
the link is in the parentheses.
...
`perf <https://perf.wiki.kernel.org/index.php/Main_Page>`__ is powerful:
it can instrument CPU performance counters, tracepoints, kprobes, and
uprobes (dynamic tracing). It is capable of lightweight profiling.
...
kprobes and uprobes frameworks, for kernel and userspace dynamic tracing

To use perf on your linux machine, assuming we are working with
*myCode.c*:

1. Compile the code (note that the -g flag tells the compiler to compile
...
3. Access the report in the directory that you ran perf record (accesses
   the outputted *perf.data* file):

   ::

       perf report

4. More can be viewed using the annotated viewer in the same directory
   with
...
If in doubt, use the included manual to figure out what to do, from the
command line run:
...
`The GNU Profiler
(GPROF) <http://web.archive.org/web/20141129061523/http://www.cs.utah.edu/dept/old/texinfo/as/gprof.html#SEC2>`__
can be used to determine which parts of a program are taking most of the
execution time.

To use it, all we do is add some additional flags to the compiler,
whether we are using CC or GCC both should work. Assuming that we’re
working with a file called *myCode.c* we would use it as follows:
...
3. Running the code should output a file called *gmon.out* to the
   ...
   output to a file called report.txt is optional:

   ::

       gprof myCode_profile.o gmon.out > report.txt

**SO WHAT?** Pending further experimentation, this looks like it will be
...
power analysis aspects of the code.
...
`VALGRIND <https://valgrind.org/docs/manual/quick-start.html>`__ is a
suite of debugging and profiling tools.

**MEMCHECK**

The most popular of the VALGRIND tools is called Memcheck. It can detect
many memory-related errors that are common in C and C++ programs and
that can lead to crashes and unpredictable behaviour.

To use it on your linux machine, assuming we are working with
*myCode.c*:
...
There’s plenty more information in the `Valgrind
Manual <https://valgrind.org/docs/manual/manual.html>`__

**CACHEGRIND**

`Cachegrind and
cgannotate <https://valgrind.org/docs/manual/cg-manual.html>`__
...
`OPROFILE <https://oprofile.sourceforge.io/faq/>`__ is an open source
...
turning profile data into human readable information are available.

OProfile is one of the tools google reported that they use in their
datacentre performance profiling in `Their 2010
Paper <https://github.com/osullik/summer2023/blob/main/Papers/bibliography/reference_papers/Ren2010.pd>`__
...
`VTUNE <https://www.intel.com/content/www/us/en/developer/tools/oneapi/vtune-profiler.html#gs.11c0nr>`__
...
profiling SYCL\ *, C, C++, C#, Fortran, OpenCL™ code, Python*, Google
...
It appears to have a GUI and some pretty hefty monitoring functions,
this one will be worth exploring further.
...
`SNIPER <https://snipersim.org/w/The_Sniper_Multi-Core_Simulator>`__ is
...
within 25% at a simulation speed of up to several MIPS.
...
Intel’s Gather, Apply, Scatter, Compute-on-Target performance model
...
their projections of PiUMA performance. `Source: HIVE Workload Analysis
Report pg
18 <https://drive.google.com/file/d/1qM5POYo0vB9p-QKb88oNzzED2a7cJdW_/view?usp=drive_link>`__

Piuma Analytic Performance Model
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

`Source: HIVE Workload Analysis Report pgs
20-22 <https://drive.google.com/file/d/1qM5POYo0vB9p-QKb88oNzzED2a7cJdW_/view?usp=drive_link>`__
The PiUMA analytic performance model seeks to determine the limiting
resource. It uses the :math:`MAX` operation because the implementation
...
memory, resulting in an overlap between then :math:`T_{net}` and
:math:`T_{comp}` (and hence drives use of the MAX value).

.. math:: T_{estimate} = MAX(T_{mem},T_{comp},T_{net}) + T_{barrier}

**Memory Access Time Calculation** :math:`T_{mem}` is the time taken to
...
are a sequence of multiple kernels. As a result, :math:`T_{mem}` will be
the sum of each of the component phases.

.. math:: T_{mem}={Bytes Accessed Mem \over mem_{bw}}

**Network Access Time Calculation** :math:`T_{net}` is the amount of
traffic crossing the bisection of the network.

.. math:: T_{net}={BytesThroughBisection \over netBisection_{bw}}

**Compute Time** :math:`T_{comp}` is modelled on the effective
...
the average latency needs to be < 16 cycles. :math:`n_iter` here is the
number of times a loop body is executed

.. math:: T_{comp}={nIter * avgInstructionsPerIter \over IPC_{effective}}

**Barrier Time** estimated using the span of the PIUMA system at each
...
die and consecutively across the dies in a socket.
