...
The dataLoader utility is designed to ingest specifically formatted
files that define a graph data structure.

Input Format
------------

To be ingested by the dataloader the input format needs to be as
follows:
...
The parser looks for the #TITLE: and #SOURCE: tags to omit those files.
...
The code is still quite brittle, so this input format must be covered
exactly.

Another **GOTCHA** to be aware of: the edge definitions are created by
...
from **node2** to **node3**, NOT node 1 to node 2.
...
Which we would of course need to compile by passing the correct files to
the compiler (here assuming GCC)
